...
(: show ratio of poetry and prose in texts of dbs croala and latty :)
...
declare function croala:poepros () { let $cat := element tbody {
(: ratio of p's and l's in texts :)
for $i in (collection("croala")|collection("latty"))//*:text[not(descendant::*:text)]
...
return element tr {
  element td {
    attribute class {db:name($i)},
    db:name($i)
  },
  element td {
    element a {
      attribute href {
    croala:solraddr(db:name($i),db:node-pre($i))
  },
  db:path($i)
}
  },
...
let $d := if ((collection("croala")|collection("latty"))//*:text[descendant::*:sp]) then count((collection("croala")|collection("latty"))//*:text[descendant::*:sp])
else "NULLUM"
...
(:~
 ...
 :)
declare
  %rest:path("/croalalatty/poepros")
  %rest:GET
  function page:poepros()
{
  (: HTML template starts here :)

<html>
<head><title>CroALa et LatTy: poesis prosa drama</title>
...
<body text="#000000">
<h1>CroALa et LatTy: poesis prosa drama</h1>
