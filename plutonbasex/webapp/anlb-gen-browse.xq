...
declare %rest:GET %rest:POST %rest:path("anlb-gen-browse")
function page:anlb-gen-browse() { 
...
let $search := "anlb-work-display-sbj?nomq=REPLACE2"
let $r := for $a in collection("anlb")//*:rc/*:s
...
element name { element a { attribute href {replace($search,'REPLACE2', $n)}, replace($n, 'SUBJECT?:? *', '') } } } , 
element td {
element num { count($a) } }
}
