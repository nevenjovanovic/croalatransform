...
declare %rest:GET %rest:POST %rest:path("anlb-work-display-sbj")
%rest:query-param("nomq", "{$nomq}")
function page:anlb-work-display-sbj($nomq as xs:string) { 
...
let $r := 
for $a at $count in collection("anlb")//*:rc[*:s=$nomq]
let $aut := $a/*:a
let $t := $a/*:t
let $st := $a/*:st
let $s := $a/*:s
