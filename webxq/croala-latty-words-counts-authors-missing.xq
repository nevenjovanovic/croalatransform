...
(: find number of particular words missing in authors in two db's and their total words :)
...
declare function croala:wordsmiss () {
...
for $db in ("latty", "croala")
 for $auctores in distinct-values(collection($db)/*:TEI/descendant::*:titleStmt/*:author/@key)
  ...
  let $count := sum(
    ...
    let $entry := sum(for $e in $verba return ft:count($e//*[not(text() contains text {$term})]))
return if ($entry=0) then element tr { 
  element td { 
    attribute class { $db },
    $db } ,
    element td { 
...
for $rr in $rijeci/tr
order by $rr/td[3] , number($rr/td[5]) descending
return $rr
...
(:~
 ...
 :)
declare
  %rest:path("/verba/desunt")
  %rest:GET
  function page:verbaqdesunt()
{
  (: HTML template starts here :)

<html>
<head><title>CroALa &amp; LatTy: verba quaedam desunt apud auctores</title>
...
<body text="#000000">
<h1>CroALa &amp; LatTy: verba quaedam desunt apud auctores</h1>

<p><a href="http://www.ffzg.unizg.hr/klafil/croala">CroALa</a>, { current-date() }. Quaerimus verba quaedam quae desunt apud auctores collectionum CroALa et LatTy. Verba sunt haec: { string-join(croala:quaedam(), ', ')}.</p>
