...
let $vrb := for $r in $rr 
return element td { if ($i//wds[v=$r]) then sum(data($i//wds[v=$r]/vc)) else 0 }


  
return element tr {
  element td { data($txtid) || data($fname) } ,
  element td { fn:number(replace(data($perd), '[^0-9]', '')) div 10 } ,
  element td { sum(for $a in $gend return map:get($genre,$a)) } ,
  element td {$hapax},
  element td { round($hapax div $twc * 100) div 100 },
  $vrb (: ,
  element td {
