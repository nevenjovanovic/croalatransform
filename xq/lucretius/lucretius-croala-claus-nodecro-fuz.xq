...
declare function croala:allud($t) {
  for $i in $t/cl
  where $i[fz/cn!=0]
  return element ln { $i/@id , 
  $i/ft/nod ,
  element fuz { data($i/fz/nod) } }
};

declare function croala:croalan ($croaladata) {
  for $lucretid in $croaladata
return for $nn in tokenize(data($lucretid), ' ') 
...
let $t := collection("lucretius-croala2")/idx
for $skup in croala:allud($t)
let $lc := croala:lucretn(data($skup/@id))
return element table { $lc , 
element tr {
  element td { count(for $a in tokenize(data($skup/nod), ' ') return $a)} ,
  element td {
  croala:croalan(data($skup/nod)) }
} ,
element tr {
  element td { count(for $a in tokenize(data($skup/fuz), ' ') return $a) } ,
  element td { croala:croalan(data($skup/fuz) ) }
} }