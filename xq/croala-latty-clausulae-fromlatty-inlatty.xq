(: using a list of clausulae in LatTy occurring in CroALa, return names of documents and node ids in LatTy :)
...
for $link in collection("lattyclausulae")//c[text() contains text {$uniq} using fuzzy]/@id
return 
element tr {
  element td { $uniq } ,
  element td {
    element a {
  attribute href { "http://solr.ffzg.hr/basex/node/latty/" || data($link) },
db:path(db:open-pre("latty", data($link)))
}

},
element td {
  db:open-pre("latty", data($link))
}
}
