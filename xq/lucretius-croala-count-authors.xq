(: count reuses of Lucretian clausulae in CroALa :)
...
let $br := //tr[2]/td[2]/a/text()
let $cnt := count(index-of($br,$f))
order by $cnt descending , $per , $f
return substring-before($f,'.xml') || "," || substring-before(replace($per, 'xx_', '.'), '_third') || "," || $cnt
