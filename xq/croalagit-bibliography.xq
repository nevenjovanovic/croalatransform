...
element m { 
for $file in collection("croalagit")
...
let $size := file:size(string-join(('/home/neven/rad/croala-r/croalagit/',substring(document-uri($file), 11)),''))
...
element item {(attribute class { ("filename")}), data(substring(document-uri($file), 11))}, 
