(: write unique 2-word clausulae to a db -- for poetiditalia :)
(: order descending :)
...
let $body :=
element tbody { 
for $c in collection("pdillfvclau2")/claus/*:c
let $un := $c/text()
...
return db:create("pdillfvuniq2", $body , "pdillclausulae1uniq.xml", map { 'ftindex': true(), 'intparse': true(), 'stripns': true() })